...
class _FormCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const ManualCard(
      ...
      title: 'กรอกข้อมูล',
      rows: [
        ManualRow('ชื่อ', 'ต้องกรอก อย่างน้อย 2 ตัวอักษร'),
        ManualRow(
          'เบอร์โทร',
          'ต้องกรอก มีตัวเลขอย่างน้อย 9 หลัก พิมพ์ขีดหรือเว้นวรรคคั่นได้',
        ),
        ManualRow('อีเมล', 'ต้องกรอก ต้องมี @ และโดเมนที่มีจุด'),
        ...
        ManualRow(
          'รูปโปรไฟล์',
          'กดปุ่มเลือกรูปจากเครื่องที่หัวฟอร์ม รูปจะถูกย่อให้เล็กก่อนเก็บอัตโนมัติ '
              'หรือจะเลือกเป็นอีโมจิกับสีแทนก็ได้ ถ้าไม่เลือกอะไรเลย'
              'จะใช้ตัวอักษรแรกของชื่อ',
        ),
      ],
    );
  }
...
class _TagCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ManualCard(
      ...
      title: 'กลุ่มทั้งห้า',
      // สร้างจาก ContactTag โดยตรง เพิ่มกลุ่มใหม่เมื่อไหร่คู่มือขึ้นให้เอง
      rows: [
        for (final tag in ContactTag.values)
          ManualRow(tag.label, _describe(tag)),
      ],
      note: 'แต่ละกลุ่มมีสีประจำตัว ใช้เป็นขีดสีซ้ายของการ์ดรายชื่อ '
          'และเป็นสีในกราฟสัดส่วนบนแดชบอร์ด',
    );
  }

  String _describe(ContactTag tag) {
    return switch (tag) {
      ContactTag.general => 'ค่าเริ่มต้นถ้าไม่ได้เลือกกลุ่ม',
      ContactTag.family => 'คนในบ้านและญาติ',
      ContactTag.friend => 'เพื่อนสนิทและคนรู้จัก',
      ContactTag.work => 'เพื่อนร่วมงาน ลูกค้า คู่ค้า',
      ContactTag.study => 'เพื่อนร่วมชั้น อาจารย์',
    };
  }
}
...
class _StorageCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const ManualCard(
      ...
      title: 'ข้อมูลเก็บที่ไหน',
      rows: [
        ManualRow(
          'บนเบราว์เซอร์',
          'อยู่ใน IndexedDB ของเบราว์เซอร์ ไม่มีไฟล์ให้เปิด ผูกกับที่อยู่เว็บที่ใช้เปิด',
        ),
        ManualRow('บนมือถือ', 'ไฟล์ my_directory.db ในโฟลเดอร์ databases ของแอป'),
        ManualRow(
          'ตาราง contacts',
          'เวอร์ชัน 5 มี 16 คอลัมน์ อัปเกรดด้วย ALTER TABLE ข้อมูลเดิมไม่หาย',
        ),
        ManualRow(
          'ตาราง settings',
          'เก็บค่าตั้งค่าของแอปแบบคีย์-ค่า อยู่ในไฟล์ฐานข้อมูลเดียวกับรายชื่อ',
        ),
      ],
      note: 'ข้อมูลบนเว็บผูกกับที่อยู่เว็บ ถ้าเปลี่ยนพอร์ตหรือโดเมน '
    ...
    return const ManualCard(
      ...
      faqs: [
        ...
        ManualFaq(
          ...
          'หน้าจอจะบอกว่าลองโหมดไหนไปแล้วบ้างและติดตรงไหน '
              'บนเบราว์เซอร์มักเกิดจากไฟล์ SQLite คนละเวอร์ชันกับโค้ด ต้อง build ใหม่',
        ),
        ManualFaq(

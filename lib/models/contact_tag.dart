...
class ContactTag {
  ...
  /// กลุ่มที่ใส่มาให้ตั้งแต่ติดตั้งครั้งแรก
  ///
  /// เก็บไว้แค่บอกที่มา ไม่ได้ล็อกอะไร ผู้ใช้แก้ชื่อ สี ไอคอน และลบได้เหมือนกลุ่มที่สร้างเอง
  ...
  /// แปลงชื่อกลุ่มเป็นข้อมูลกลุ่ม ถ้าไม่เจอให้ตกมาที่กลุ่มแรกที่ยังมีอยู่
  ///
  /// ตกมาที่กลุ่มแรกแทนที่จะเป็น "ทั่วไป" ตายตัว เพราะผู้ใช้ลบกลุ่มไหนก็ได้
  /// รวมถึง "ทั่วไป" ด้วย ถ้าคืนกลุ่มที่ถูกลบไปแล้วจะไปโผล่ในเมนูเลือกกลุ่มทั้งที่ไม่มีอยู่จริง
  static ContactTag fromLabel(String? label) {
    return all.firstWhere(
      (tag) => tag.label == label,
      orElse: () => all.isEmpty ? general : all.first,
    );
  }
